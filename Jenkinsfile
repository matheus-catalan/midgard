pipeline {
    ...
    stages {
        stage ('Build Image') {
            steps {
                script {
                    slackSend(color: "good", message: "[${GROOVY,script = 'String.format('%tF %<tH:%<tM', java.time.LocalDateTime.now())'}] <${env.BUILD_URL}|Build Started - ${env.BUILD_NUMBER}>")
    ...
    post {
        success {
            slackSend(color: "good", message: "[${GROOVY,script = 'String.format('%tF %<tH:%<tM', java.time.LocalDateTime.now())'}] <${env.BUILD_URL}|Build deployed successfully - ${env.BUILD_NUMBER}>")
        }
        failure {
            slackSend(color: "#f70505", failOnError:true, message:"[${GROOVY,script = 'String.format('%tF %<tH:%<tM', java.time.LocalDateTime.now())'}] <${env.BUILD_URL}|Build failed  - ${env.BUILD_NUMBER} >")
        }
    }
}
