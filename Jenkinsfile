        ...
        stage ('Run Containers to test') {
            steps {
                script {
                    ...
                    docker.image('postgres:13').run('--rm --name db-test --network=cosmos_network -e POSTGRES_DB=test -e POSTGRES_USER=test -e POSTGRES_PASSWORD=test -p 5433:5432 -d', '-p 5433')
