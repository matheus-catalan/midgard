        ...
        stage ('Run Containers to test') {
            steps {
                script {
                    ...
                    docker.image('postgres:13').run('--rm --name db-midgard --network=cosmos_network -e POSTGRES_DB=test -e POSTGRES_USER=test -e POSTGRES_PASSWORD=test -p 5432:5432 -d', '-p 5432')
